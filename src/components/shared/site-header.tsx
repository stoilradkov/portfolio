...
export function SiteHeader({ homePage = false }: SiteHeaderProps) {
  ...
  const resolveHref = (sectionId: string) => (homePage ? `#${sectionId}` : homeAnchor(sectionId));

  return (
    <header className="sticky top-0 z-50 bg-background/92 backdrop-blur-xl lg:fixed lg:inset-y-0 lg:left-0 lg:w-[17.5rem] lg:bg-transparent lg:backdrop-blur-0">
      <div className="mx-auto flex w-full max-w-[61rem] items-center justify-between px-5 py-5 sm:px-6 lg:h-full lg:max-w-none lg:flex-col lg:items-start lg:px-8 lg:py-8">
        <div className="flex items-center gap-5 lg:flex-col lg:items-start lg:gap-9">
          <a aria-label={`${siteContent.name} home`} href={homePage ? "#top" : homeAnchor()}>
            <img
        ...
        <div className="hidden items-center gap-5 text-sm md:flex lg:flex-col lg:items-start lg:gap-3">
          <a className="text-muted-foreground transition-colors hover:text-foreground" href={siteContent.contact.linkedin} target="_blank" rel="noreferrer">
            LinkedIn
          </a>
          <a className="text-muted-foreground transition-colors hover:text-foreground" href={`mailto:${siteContent.contact.email}`}>
            Email
          </a>
        </div>
      ...
      >
        <div className="mx-auto flex w-full max-w-[61rem] flex-col gap-2 px-5 py-4 sm:px-6 lg:px-8">
          {navigation.map((item) => (
            <a
              key={item.sectionId}
          ...
          <div className="mt-3 grid gap-2 pt-3 text-sm">
            ...
            <a className="rounded-2xl px-4 py-3 text-foreground hover:bg-accent" href={`mailto:${siteContent.contact.email}`}>
              Email
            </a>
          </div>
        </div>
